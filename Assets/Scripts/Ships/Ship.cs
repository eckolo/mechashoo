...
public partial class Ship : Things {
    ...
    protected void setParamate() {
        ...
        for(var index = 0; index < weaponSlots.Count; index++) {
            ...
            if(weaponSlots[index].entity == null)
                continue;
    ...
    public void setAllAlignment(Vector2 setPosition) {
        ...
        foreach(var arm in armStates)
            arm.alignment = setPosition;
    }
    ...
    public float receiveDamage(float damage, bool penetration = false, bool continuation = false) {
        ...
        if(!penetration)
            palamates.nowBarrier = Mathf.Max(palamates.nowBarrier - damage, 0);
        ...
        if(surplusDamage > 0) {
            ...
            if(!continuation)
                GetComponent<SpriteRenderer>().color = new Color(0f, 1f, 0.6f, 1);
        }
    ...
    protected Hand getHand(Parts target) {
        if(target == null)
            return null;
        if(target.childParts == null)
            return target.GetComponent<Hand>();
    ...
    public void stopAllWeapon() {
        foreach(var armstate in armStates) {
            ...
            if(hand != null)
                hand.actionWeapon(Weapon.ActionType.NOMOTION);
        }
    }
